...
func getPackageCommandUsage(command string) string {
	switch command {
	case "install":
		return "orb install <source> [-l] [--approve|--no-approve]"
	case "remove":
		return "orb remove <source> [-l] [--approve|--no-approve]"
	case "update":
		return "orb update [target] [--self|--extensions|--models|--all] [--extension <source>] [--approve|--no-approve]"
	default:
		return "orb list [--approve|--no-approve]"
	}
}
...
Examples:
  orb install npm:@foo/bar
  orb install git:github.com/user/repo
  orb install git:git@github.com:user/repo
  orb install https://github.com/user/repo
  orb install ssh://git@github.com/user/repo
  orb install ./local/path
...
Examples:
  orb remove npm:@foo/bar
  orb uninstall npm:@foo/bar
...
func parsePackageCommand(args []string) *packageCommandOptions {
	...
	if command == "update" {
		...
		case options.source != "":
			if options.source == "self" || options.source == "orb" {
				if extensionsFlag {
...
func handleConfigCommand(ctx context.Context, argv []string, streams cliStreams, dependencies cliDependencies) (bool, int) {
	...
	for _, arg := range rest {
		...
		case strings.HasPrefix(arg, "-"):
			...
			_, _ = fmt.Fprintf(streams.Stderr, "Use \"orb --help\" or \"%s\".\n", configCommandUsage)
			return true, 1
		default:
	...
	if !streams.StdinTTY || !streams.StdoutTTY {
		_, _ = fmt.Fprintln(streams.Stderr, "Error: orb config requires an interactive terminal (stdin and stdout must be TTYs).")
		return true, 1
	}
