...
func TestRunCLIVersionIncludesUpstreamIdentity(t *testing.T) {
	...
	want := fmt.Sprintf("orb %s (upstream pi %s @ %.8s)\n", version, lock.Version, lock.Commit)
...
func TestRunCLIChatCommand(t *testing.T) {
	...
	t.Setenv("ORB_CHAT_ALLOWED_SENDERS", "")
	...
	stdout.Reset()
	t.Setenv("ORB_CHAT_ALLOWED_SENDERS", "123")
	...
	t.Setenv("ORB_CHAT_PATH", "missing-slash")
...
func TestBuiltBinaryDispatchesHelp(t *testing.T) {
	...
	root := t.TempDir()
	command.Env = append(os.Environ(), config.EnvAgentDir+"="+filepath.Join(root, "agent"), "ORB_STATE_HOME="+filepath.Join(root, "state"), "ORB_BRIDGE_HOME="+filepath.Join(root, "bridge"))
	output, err := command.CombinedOutput()
	if err != nil {
		t.Fatalf("run binary: %v\n%s", err, output)
	}
	if !bytes.Contains(output, []byte("Usage: orb")) || !bytes.Contains(output, []byte("--continue")) {
