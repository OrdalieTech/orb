...
// Harness storage owns generic session persistence; agent/session remains an upper-layer wire manager.
type SessionEntry struct {
	...
	RetainedTail     engine.AgentMessages
...
type CompactionPreparation struct {
	FirstKeptEntryID    string
	MessagesToSummarize engine.AgentMessages
	TurnPrefixMessages  engine.AgentMessages
	RetainedTail        engine.AgentMessages
	IsSplitTurn         bool
	TokensBefore        int64
...
func (result CompactionResult) MarshalJSON() ([]byte, error) {
	var retainedTail *engine.AgentMessages
	...
	return ai.Marshal(struct {
		...
		RetainedTail     *engine.AgentMessages `json:"retainedTail,omitempty"`
		Details          any                   `json:"details,omitempty"`
	}{
