...
type CustomEntryContextMessageProjector func(SessionTreeEntry, int, []SessionTreeEntry) engine.AgentMessages
...
func decodeHarnessAgentMessages(values []json.RawMessage) engine.AgentMessages {
	if values == nil {
		return nil
	}
	messages := make(engine.AgentMessages, 0, len(values))
