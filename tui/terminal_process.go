...
func (terminal *ProcessTerminal) Start(onInput func(string), onResize func()) error {
	...
	reader, err := openTerminalReader(terminal.input, terminal.output)
	if err != nil {
		_ = term.Restore(int(terminal.input.Fd()), state)
		terminal.mu.Unlock()
		return err
	}
	terminal.rawState, terminal.readInput = state, reader
	...
	terminal.stopResize = watchTerminalResize(terminal, func() {
		terminal.mu.Lock()
	...
	go guarded(func() {
		...
		for {
			...
			ready, waitErr := reader.wait(terminalReadPollInterval)
			if waitErr != nil {
				return
			}
			if !ready {
...
func (terminal *ProcessTerminal) dispatchInput(sequence string) {
	...
	terminal.mu.Unlock()
	if active && handler != nil {
		detectShiftEnter := nativeShiftEnterDetection && sequence == "\r"
		handler(NormalizeAppleTerminalInput(sequence, detectShiftEnter, detectShiftEnter && nativeShiftPressed()))
	}
}
...
func (terminal *ProcessTerminal) Stop() error {
	...
	if terminal.stopResize != nil {
		terminal.stopResize()
		terminal.stopResize = nil
	}
