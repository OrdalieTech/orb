...
- Memory plugin instances no longer share one process-wide store lock: each `MemoryWithStore` instance serializes its own operations, so multi-tenant embedders' stores serve concurrent queries across instances. Cross-instance and cross-process coherence remains the `memory.Store`'s responsibility, as durable adapters already require.
