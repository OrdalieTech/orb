...
async function buildWriteFixture(session: SessionModule) {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "orb-f6-"));
  try {
...
async function buildInvalidUTF8Fixture(session: SessionModule): Promise<InvalidUTF8Fixture> {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "orb-f6-invalid-utf8-"));
  try {
...
async function buildTreeFixture(session: SessionModule) {
  ...
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "orb-f6-branch-persistence-"));
  try {
    return {
      before,
      branched,
...
async function buildForkFixture(session: SessionModule) {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "orb-f6-fork-"));
  const OriginalDate = globalThis.Date;
  try {
...
async function buildListFixture(session: SessionModule) {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "orb-f6-list-"));
  try {
...
async function buildExportFixture(upstreamRoot: string, exportModule: ExportModule) {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), "orb-f6-export-"));
  try {
    const input =
