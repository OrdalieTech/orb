...
async function extractOpenAIProvider(upstreamRoot: string): Promise<ProviderFixture> {
  ...
  const temporaryRoot = await mkdtemp(path.join(tmpdir(), "orb-f2-openai-provider-"));
...
export async function generateF2(upstreamRoot: string, outputRoot: string, upstreamCommit: string): Promise<void> {
  ...
  try {
    ...
    const bedrock = await extractBedrockF2(upstreamRoot);
    ...
    const manifest = {
      ...
      files: [
        ...
        "azure-requests.json",
        "azure-streams.json",
        "pi-messages-requests.json",
        "pi-messages-streams.json",
        "auth-storage.json",
      ],
    };
    ...
    await writeFile(
      path.join(familyDir, "bedrock-provider.json"),
      `${JSON.stringify(bedrock.provider, null, 2)}\n`,
    );
    ...
    await writeFile(
      path.join(familyDir, "bedrock-requests.json"),
      `${JSON.stringify({ cases: bedrock.requests }, null, 2)}\n`,
    );
    await writeFile(
    ...
    await writeFile(
      path.join(familyDir, "bedrock-streams.json"),
      `${JSON.stringify({ cases: bedrock.streams }, null, 2)}\n`,
    );
    await writeFile(
    ...
    await writeFile(
      path.join(familyDir, "pi-messages-requests.json"),
      `${JSON.stringify({ cases: piMessages.requests }, null, 2)}\n`,
    );
    await writeFile(
      path.join(familyDir, "pi-messages-streams.json"),
      `${JSON.stringify({ cases: piMessages.streams }, null, 2)}\n`,
    );
