...
Two tiers exist since the D35 split (2026-08-11). The render-golden families — every `F12*` directory plus the `WP450` replay/preview/UI-demo files — are Orb-owned snapshots of Orb's own renderer: they are excluded from upstream extraction and from the `fixtures-check` diff, guarded instead by their Go comparison tests, and regenerated only by the explicit `make fixtures-tui` (`ORB_UPDATE_F12=1`), which rewrites presentation values (rendered frames, highlight output, theme colors, layout) in place while behavior-shaped values in the same files stay frozen upstream captures. Their `manifest.json` files keep the provenance of the original upstream capture. Every other family remains an upstream-extracted pi-parity gate.
...
`orb --mode rpc` executable. An untagged production-binary conversation separately drives a local
...
WP-450 added the `WP450` side-by-side replay family, originally extracted by driving upstream's
real interactive components through its `@xterm/headless@5.5.0` `VirtualTerminal` at two widths and
recording every settled visible frame plus the documented status-line, widget-placement,
custom-header, and custom-footer examples. Since the D35 split its render frames are Orb-owned
snapshots regenerated by `make fixtures-tui`; the recorded status-line events and retention flags
stay frozen upstream captures.
