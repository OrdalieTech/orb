...
func TestAzureOpenAIResponsesLiveToolCallRoundTrip(t *testing.T) {
	if os.Getenv("ORB_LIVE_TESTS") != "1" {
		t.Skip("set ORB_LIVE_TESTS=1 to run the Azure OpenAI live smoke test")
	}
	apiKey := os.Getenv("AZURE_OPENAI_API_KEY")
	if apiKey == "" || (os.Getenv("AZURE_OPENAI_BASE_URL") == "" && os.Getenv("AZURE_OPENAI_RESOURCE_NAME") == "") {
		t.Fatal("ORB_LIVE_TESTS=1 requires AZURE_OPENAI_API_KEY and AZURE_OPENAI_BASE_URL or AZURE_OPENAI_RESOURCE_NAME")
	}
	modelID := os.Getenv("ORB_AZURE_OPENAI_MODEL")
	if modelID == "" {
		modelID = "gpt-4o-mini"
	}
	model := &ai.Model{
	...
	messages := ai.MessageList{&ai.UserMessage{
		Content: ai.NewUserText("Call the echo tool exactly once with text orb-live. Do not answer until the tool result arrives."), Timestamp: time.Now().UnixMilli(),
	}}
	...
	if call.Name != "echo" || call.Arguments["text"] != "orb-live" {
		t.Fatalf("tool call = %q %#v, want echo with orb-live", call.Name, call.Arguments)
	}
	messages = append(messages, toolRequest, &ai.ToolResultMessage{
		ToolCallID: call.ID, ToolName: call.Name,
		Content: ai.ToolResultContent{&ai.TextContent{Text: "orb-live"}}, Timestamp: time.Now().UnixMilli(),
	})
