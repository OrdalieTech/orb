...
func TestBedrockConverseStreamLiveToolCallRoundTrip(t *testing.T) {
	if os.Getenv("ORB_LIVE_TESTS") != "1" {
		t.Skip("set ORB_LIVE_TESTS=1 to run the Amazon Bedrock live smoke test")
	}
	if !hasLiveBedrockCredentials() {
		t.Fatal("ORB_LIVE_TESTS=1 requires a Bedrock bearer token, AWS profile, IAM keys, ECS task role, or web identity")
	}

	modelID := os.Getenv("ORB_BEDROCK_MODEL")
	if modelID == "" {
	...
	required := &api.BedrockToolChoice{Type: "tool", Name: "echo"}
	region := os.Getenv("ORB_BEDROCK_REGION")
	sdkBackend := Backend()

	ctx, cancel := context.WithTimeout(context.Background(), 90*time.Second)
	defer cancel()
	first, err := api.StreamBedrockConverseWithOptions(ctx, model, ai.Context{Messages: messages, Tools: &tools}, &api.BedrockConverseStreamOptions{
		StreamOptions: ai.StreamOptions{MaxTokens: &maxTokens, TimeoutMS: &timeoutMS}, Region: region, ToolChoice: required, Backend: &sdkBackend,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	second, err := api.StreamBedrockConverseWithOptions(ctx, model, ai.Context{Messages: messages, Tools: &tools}, &api.BedrockConverseStreamOptions{
		StreamOptions: ai.StreamOptions{MaxTokens: &maxTokens, TimeoutMS: &timeoutMS}, Region: region, Backend: &sdkBackend,
	})
	if err != nil {
		t.Fatal(err)
	}
