...
Lane B: everything that makes orb a complete headless product and embeddable SDK.
...
**Acceptance:** trace fixtures green; `orb --mode json` drives a faux session identically to upstream.
...
**Fixtures:** **F7** — recorded upstream RPC transcripts replayed against `orb --mode rpc`;
PLUS upstream's own RPC test files executed unmodified against the Go binary via the WP-002 adapter.

**Acceptance:** F7 green; upstream RPC test suite passes against orb (exclusions documented
...
**Scope:** `orb install/remove/update/list/config` for `npm:` (registry tarball fetch + extract,
