...
localhost :53692 callback server, manual code-paste fallback), token refresh; headless `orb login
anthropic` / `orb logout` (TUI `/login` arrives Phase 4 on the same core).
...
file, mirroring upstream's correction mechanism); runtime `orb update --models` refresh from
