...
func newRPCDispatchRuntime(t *testing.T, sessionID string) *agent.SessionRuntime {
	t.Helper()
	root := t.TempDir()
...
func TestRPCBashHonorsUserBashInterception(t *testing.T) {
	...
	mode := &server{ctx: context.Background()}
	response := mode.handleCommand(runtime, Command{Type: "bash", Command: "printf local"})
