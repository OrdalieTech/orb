...
func TestReadStrictJSONLinesMatchesUpstreamFraming(t *testing.T) {
	...
	go ReadFrames(strings.NewReader(input), lines, errors)
	var got []string
...
func TestRPCSlashCommandWireOrderAndOptionalEmptyValues(t *testing.T) {
	empty := ""
	encoded, err := ai.Marshal(SlashCommand{
		Name: "skill:test", Description: &empty, Source: "skill",
		SourceInfo: SourceInfo{Path: "test/SKILL.md", Source: "skills", Scope: "project", Origin: "top-level", BaseDir: &empty},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRPCExtensionUIOptionalFieldsPreserveEmptyValues(t *testing.T) {
	requests := make(chan ExtensionUIRequest, 3)
	ui := newExtensionUI(func(value any) error {
		requests <- value.(ExtensionUIRequest)
		return nil
	})
	defer ui.close()
	...
	ui.HandleResponse(ExtensionUIResponse{ID: input.ID, Cancelled: true})
	<-inputDone
	...
	ui.HandleResponse(ExtensionUIResponse{ID: selectRequest.ID, Cancelled: true})
	<-selectDone
...
func TestRPCGetAvailableThinkingLevels(t *testing.T) {
	...
	exitCode := Serve(context.Background(), &rpcTestHost{runtime: runtime}, Options{
		Input:  strings.NewReader("{\"id\":\"levels\",\"type\":\"get_available_thinking_levels\"}\n"),
