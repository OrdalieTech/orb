...
func TestSessionRuntimeFinalBashFlushWaitsForRecording(t *testing.T) {
	...
	runtime := &SessionRuntime{agent: engine.NewAgent(nil), manager: manager}
	released := false
	release := func() {
		if !released {
...
func TestSessionRuntimeDefaultCompletionAppliesRequestAuth(t *testing.T) {
	...
	created := engine.NewAgent(nil, engine.WithInitialState(engine.AgentState{
		Model: provider.GetModel(), SystemPrompt: "test", Messages: engine.AgentMessages{}, Tools: []engine.AgentTool{},
	}))
	...
	stream := func(
		...
		if value, exists := options.Headers["x-api-key"]; !exists || value != nil {
			t.Fatalf("nullable completion auth header was not preserved: %#v", options.Headers)
		}
...
func newTestRuntimeWithHeaders(t *testing.T, provider *faux.Provider, settings map[string]any, getModelHeaders engine.GetModelHeadersFunc) (*SessionRuntime, *sessionstore.SessionManager) {
	t.Helper()
	root := t.TempDir()
	agentDir := filepath.Join(root, "agent")
	if err := os.MkdirAll(agentDir, 0o755); err != nil {
	...
	created := engine.NewAgent(
		provider.StreamSimple, engine.WithInitialState(engine.AgentState{Model: provider.GetModel(), SystemPrompt: "test", Messages: engine.AgentMessages{}, Tools: []engine.AgentTool{}}),
