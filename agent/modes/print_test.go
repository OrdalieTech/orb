	...
	for _, stopReason := range []ai.StopReason{ai.StopReasonError, ai.StopReasonAborted} {
		t.Run(string(stopReason), func(t *testing.T) {
			...
			session := &jsonPrintSession{
				state: engine.AgentState{Messages: engine.AgentMessages{message}},
				events: []any{
					engine.AgentStartEvent{},
					agent.SessionAgentEndEvent{Messages: engine.AgentMessages{message}, WillRetry: false},
...
func (session *jsonPrintSession) State() engine.AgentState { return session.state }
...
func TestRunPrintModeJSONWaitsForQueuedOutputBeforeReturning(t *testing.T) {
	...
	session := &jsonPrintSession{events: []any{engine.AgentStartEvent{}, agent.AgentSettledEvent{}}}
...
func TestRunPrintModePromptErrorReturnsOne(t *testing.T) {
	...
	session := newPrintAgent(provider)
	session.Subscribe(func(context.Context, engine.AgentEvent) error {
...
func TestRunPrintModeSignalShutdown(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			session.Subscribe(func(ctx context.Context, _ engine.AgentEvent) error {
...
func userPromptTexts(t *testing.T, state engine.AgentState) []string {
	t.Helper()
	var prompts []string
