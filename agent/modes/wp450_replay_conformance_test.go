...
func renderWP450ReplayWidth(width int) []ConformanceReplayFrame {
	...
	footerSession := &wp450FooterSession{state: engine.AgentState{
...
func wp450HeaderFactory(_ extensions.UIHost, theme extensions.Theme) extensions.Component {
	...
	return wp450LinesComponent{
		...
		leg,
		"",
		theme.FG("muted", "   shitty coding agent") + dim(" v0.84.1"),
	}
}
...
type wp450FooterSession struct{ state engine.AgentState }

func (session *wp450FooterSession) State() engine.AgentState { return session.state }
func (*wp450FooterSession) GetSessionStats() agent.SessionStats {
	tokens := int64(1024)
	percent := 12.5
	return agent.SessionStats{ContextUsage: &harness.ContextUsage{
