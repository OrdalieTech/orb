...
func selectorResource(path string, enabled bool, source, scope, origin, baseDir string) agent.ResolvedResource {
	return agent.ResolvedResource{
		Path: path, Enabled: enabled,
		Metadata: agent.PathMetadata{Source: source, Scope: scope, Origin: origin, BaseDir: baseDir},
	}
}
...
func TestConfigSelectorScopeSwitchCloseAndExit(t *testing.T) {
	...
	closed, exited := 0, 0
	selector := NewConfigSelector(ConfigSelectorOptions{
		ResolvedPaths:   ScopedResolvedPaths{Global: &agent.ResolvedPaths{}, Project: &agent.ResolvedPaths{}},
