...
func configGroupLabel(metadata agent.PathMetadata, agentDir string) string {
...
func buildConfigGroups(resolved *agent.ResolvedPaths, agentDir string) []*configResourceGroup {
	if resolved == nil {
		resolved = &agent.ResolvedPaths{}
	}
	groupsByKey := make(map[string]*configResourceGroup)
	addResources := func(resources []agent.ResolvedResource, resourceType configResourceType) {
