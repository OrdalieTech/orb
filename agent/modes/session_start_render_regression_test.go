...
// replacement flow the way cmd/orb's interactive host does: teardown, rebind
...
type recordingSessionHost struct {
	...
	afterStart       func(*agent.SessionRuntime) error
}

func (host *recordingSessionHost) Session() *agent.SessionRuntime { return host.session }
func (host *recordingSessionHost) SetRebindSession(rebind func(*agent.SessionRuntime) error) {
	host.rebind = rebind
}
func (host *recordingSessionHost) SetBeforeSessionInvalidate(callback func()) {
	host.beforeInvalidate = callback
}
func (host *recordingSessionHost) SetAfterSessionStart(callback func(*agent.SessionRuntime) error) {
...
func newSessionStartRuntime(t *testing.T, settingsJSON map[string]any, registry *extensions.Registry, seed []any) *agent.SessionRuntime {
	t.Helper()
	cwd := t.TempDir()
...
// surfaces orb has: a replacement renders its restored session state and is
...
// (Upstream's loaded-resources container has no orb equivalent; the host
// ordering itself is pinned by cmd/orb interactive_host tests.)
