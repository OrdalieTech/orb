...
func newAnthropicWarningTestMode(t *testing.T, key string, host InteractiveSessionHost) *InteractiveMode {
	return newAnthropicWarningTestModeWithResolver(t, host, func(context.Context, ai.ProviderID) (*engine.RequestAuth, error) {
...
func newAnthropicWarningTestModeWithResolver(
	...
	resolver func(context.Context, ai.ProviderID) (*engine.RequestAuth, error),
) *InteractiveMode {
	t.Helper()
...
func TestLOGM4AnthropicWarningIsAtomicOncePerSession(t *testing.T) {
	...
	mode := newAnthropicWarningTestModeWithResolver(t, &authFlowHost{}, func(context.Context, ai.ProviderID) (*engine.RequestAuth, error) {
		started <- struct{}{}
		<-release
		return &engine.RequestAuth{APIKey: &key}, nil
	})
...
func TestLOGM4AnthropicWarningSurvivesOAuthRefreshFailure(t *testing.T) {
	...
	mode := newAnthropicWarningTestModeWithResolver(t, host, func(context.Context, ai.ProviderID) (*engine.RequestAuth, error) {
...
func TestModelSelectorSelectionIsCancelledBeforeShutdown(t *testing.T) {
	...
	release := make(chan struct{})
	mode := newAnthropicWarningTestModeWithResolver(t, nil, func(ctx context.Context, _ ai.ProviderID) (*engine.RequestAuth, error) {
		close(started)
		select {
		case <-ctx.Done():
			return nil, ctx.Err()
