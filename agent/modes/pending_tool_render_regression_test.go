...
func TestWindowedChatProductionMutationsRefreshCachedChildren(t *testing.T) {
	t.Run("assistant update", func(t *testing.T) {
		...
		mode.handleEvent(engine.MessageStartEvent{Message: assistantTextMessage("BEFORE_STREAM")})
		_ = mode.chat.LineCount(120)
		mode.handleEvent(engine.MessageUpdateEvent{Message: assistantTextMessage("AFTER_STREAM")})
...
func TestRestoredPendingToolCallsReceiveLiveCompletionEvents(t *testing.T) {
	...
	mode.handleEvent(engine.ToolExecutionEndEvent{
		ToolCallID: pendingToolCallID,
		ToolName:   pendingToolName,
		Result:     engine.AgentToolResult{Content: ai.ToolResultContent{&ai.TextContent{Text: "FINAL_RESULT"}}},
	})
