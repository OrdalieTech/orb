...
func TestRunPrintModeSubprocessExitsWithSignalCode(t *testing.T) {
	...
	command.Env = append(os.Environ(),
		"ORB_PRINT_SIGNAL_HELPER=1",
		"ORB_PRINT_SIGNAL_READY="+ready,
	)
	output := &lockedBuffer{}
...
func TestRunPrintModeSignalHelper(t *testing.T) {
	if os.Getenv("ORB_PRINT_SIGNAL_HELPER") != "1" {
	...
	session.Subscribe(func(ctx context.Context, _ engine.AgentEvent) error {
		if err := os.WriteFile(os.Getenv("ORB_PRINT_SIGNAL_READY"), []byte("ready"), 0o600); err != nil {
			return err
		}
		<-ctx.Done()
		return ctx.Err()
