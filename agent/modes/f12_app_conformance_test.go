...
func TestInteractiveSessionTitleSurvivesEventsAndExtensionReset(t *testing.T) {
	...
	mode.interactiveUI = NewInteractiveUI(mode)
	want := "orb - named session - " + filepath.Base(mode.cwd)

	mode.handleEvent(agent.SessionInfoChangedEvent{})
...
func newF12AutocompleteMode(t *testing.T, enableSkillCommands bool, extraCommands ...string) *InteractiveMode {
	...
	prompts := []agent.PromptTemplate{{
		Name: "review-prompt", Description: "Review a path", ArgumentHint: "<path>",
		FilePath:   "/fixture/prompts/review-prompt.md",
		SourceInfo: agent.SourceInfo{Path: "/fixture/prompts/review-prompt.md", Source: "local", Scope: "project", Origin: "top-level"},
	}}
	skills := []agent.Skill{{
		...
		SourceInfo: agent.SourceInfo{Path: "/fixture/skills/inspect-skill/SKILL.md", Source: "cli", Scope: "temporary", Origin: "top-level"},
	}}
	loader, err := agent.NewDefaultResourceLoader(agent.DefaultResourceLoaderOptions{
		...
		SkillsOverride: func(agent.ResourceSkillsResult) agent.ResourceSkillsResult {
			return agent.ResourceSkillsResult{Skills: skills, Diagnostics: []agent.ResourceDiagnostic{}}
		},
		PromptsOverride: func(agent.ResourcePromptsResult) agent.ResourcePromptsResult {
			return agent.ResourcePromptsResult{Prompts: prompts, Diagnostics: []agent.ResourceDiagnostic{}}
		},
	})
	if err != nil {
		t.Fatal(err)
	...
	runtime, err := agent.NewSessionRuntime(agent.SessionRuntimeConfig{
		...
		SlashResolver: &agent.SlashResolver{PromptTemplates: prompts, Skills: skills},
...
func TestF12ExtensionEditorExternalLifecycleMatchesUpstream(t *testing.T) {
	...
	t.Setenv("ORB_F12_EXTERNAL_EDITOR_LOG", lifecyclePath)
	externalEditorCommand := strings.Join([]string{os.Args[0], "-test.run=TestF12ExternalEditorHelper", "--"}, " ")
	t.Setenv("VISUAL", "orb-f12-visual-must-not-run")
	t.Setenv("EDITOR", "")
...
func TestF12ExternalEditorHelper(t *testing.T) {
	logPath := os.Getenv("ORB_F12_EXTERNAL_EDITOR_LOG")
	if logPath == "" {
		return
	}
