...
func NewLsTool(cwd string, options *LsToolOptions) engine.AgentTool {
...
func (tool *lsTool) Execute(
	...
	_ string,
	params any,
	_ engine.AgentToolUpdateCallback,
) (engine.AgentToolResult, error) {
	input, err := lsInput(params)
	if err != nil {
		return engine.AgentToolResult{}, err
	}
	return runCancelable(ctx, func() (engine.AgentToolResult, error) {
...
func (tool *lsTool) execute(ctx context.Context, input LsToolInput) (engine.AgentToolResult, error) {
	path := "."
	...
	if !exists {
		return engine.AgentToolResult{}, upstreamToolErrorf("Path not found: %s", directoryPath)
	}
	...
	if !stat.IsDirectory() {
		return engine.AgentToolResult{}, upstreamToolErrorf("Not a directory: %s", directoryPath)
	}
	entries, err := tool.operations.ReadDir(ctx, directoryPath)
	if err != nil {
		return engine.AgentToolResult{}, upstreamToolErrorf("Cannot read directory: %s", err)
	}
	...
	if len(results) == 0 {
		return engine.AgentToolResult{Content: ai.ToolResultContent{&ai.TextContent{Text: "(empty directory)"}}}, nil
	}
