...
func NewBashTool(cwd string, options *BashToolOptions) engine.AgentTool {
...
func (tool *bashTool) Execute(
	...
	if finishErr != nil {
		return engine.AgentToolResult{}, finishErr
	}
	...
	if executeErr != nil {
		...
		case message == "aborted":
			return engine.AgentToolResult{}, upstreamToolError(appendBashStatus(formatBashSnapshot(output.accumulator, snapshot, ""), "Command aborted"))
		case strings.HasPrefix(message, "timeout:"):
			timeoutSeconds := strings.Split(message, ":")[1]
			return engine.AgentToolResult{}, upstreamToolError(appendBashStatus(formatBashSnapshot(output.accumulator, snapshot, ""), "Command timed out after "+timeoutSeconds+" seconds"))
		default:
...
func (*bashTool) RenderResult(result engine.AgentToolResult) string {
...
func newBashOutputStream(onUpdate engine.AgentToolUpdateCallback) *bashOutputStream {
	prefix := "pi-bash"
...
func (stream *bashOutputStream) emitUpdateLocked() {
	...
	stream.onUpdate(engine.AgentToolResult{
...
var _ engine.AgentTool = (*bashTool)(nil)
