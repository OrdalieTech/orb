...
func (tool *editTool) Spec() engine.AgentToolSpec {
...
func (tool *editTool) Execute(
	...
	if len(input.Edits) == 0 {
		return engine.AgentToolResult{}, upstreamToolError("Edit tool input is invalid. edits must contain at least one replacement.")
	}
	absolutePath, err := ResolveToCwd(input.Path, tool.cwd)
	if err != nil {
		return engine.AgentToolResult{}, err
	}
	return withFileMutationQueueContext(ctx, absolutePath, func() (engine.AgentToolResult, error) {
		...
		if err := tool.operations.Access(ctx, absolutePath); err != nil {
			if abortErr := checkAborted(ctx); abortErr != nil {
				return engine.AgentToolResult{}, abortErr
			}
			return engine.AgentToolResult{}, editAccessError(input.Path, err)
		}
		if err := checkAborted(ctx); err != nil {
			return engine.AgentToolResult{}, err
		}
