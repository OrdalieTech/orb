...
func TestRunnerContextMiddlewareUsesDeepCopy(t *testing.T) {
	...
	original := engine.AgentMessages{map[string]any{"role": "custom", "nested": map[string]any{"value": "original"}}}
	...
	if err := registry.Register("tools", func(api API) error {
		api.RegisterTool(ToolDefinition{
			...
			Execute: func(context.Context, string, any, engine.AgentToolUpdateCallback, Context) (engine.AgentToolResult, error) {
				...
				activeMu.Unlock()
				return engine.AgentToolResult{Content: ai.ToolResultContent{}}, nil
			},
		})
		return nil
	}); err != nil {
