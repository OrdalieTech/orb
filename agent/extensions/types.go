...
type AgentEndEvent struct{ Messages engine.AgentMessages }
...
type MessageEndEvent struct{ Message engine.AgentMessage }
...
type ThinkingLevelSelectEvent struct {
	Level         engine.ThinkingLevel
	PreviousLevel engine.ThinkingLevel
}
...
type API interface {
	...
	GetThinkingLevel() (engine.ThinkingLevel, error)
	SetThinkingLevel(engine.ThinkingLevel) error
