...
func TestStateSnapshotActionsEventBusAndToolCallVeto(t *testing.T) {
	...
	actions := extensions.Actions{
		...
		GetThinkingLevel: func() (engine.ThinkingLevel, error) { return engine.ThinkingLow, nil },
	}
	...
	contextMessages := runner.EmitContext(context.Background(), engine.AgentMessages{
...
func userText(message engine.AgentMessage) string {
