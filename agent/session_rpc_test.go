...
func TestCycleModelBackwardFromAbsentCurrentModelUsesIndexZero(t *testing.T) {
	...
	outside := rpcTestModel("provider-d", "outside")
	created := engine.NewAgent(nil, engine.WithInitialState(engine.AgentState{
		Model: &outside, ThinkingLevel: ai.ModelThinkingLow, Messages: engine.AgentMessages{},
	}))
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{
		Agent: created, SessionManager: manager, Settings: settings,
...
func TestSetThinkingLevelWithUnknownModelMatchesUpstream(t *testing.T) {
	...
	created := engine.NewAgent(nil, engine.WithInitialState(engine.AgentState{ThinkingLevel: ai.ModelThinkingOff, Messages: engine.AgentMessages{}}))
...
func TestLastAssistantTextUsesECMAScriptTrim(t *testing.T) {
	...
	created := engine.NewAgent(nil, engine.WithInitialState(engine.AgentState{Messages: engine.AgentMessages{message}}))
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{Agent: created, SessionManager: manager, Settings: settings})
	if err != nil {
		t.Fatal(err)
	}
...
func TestManualCompactionWithoutModelUsesAuthGuidance(t *testing.T) {
	...
	created := engine.NewAgent(nil, engine.WithInitialState(engine.AgentState{Messages: engine.AgentMessages{userMessage("work")}}))
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{Agent: created, SessionManager: manager, Settings: settings})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPromptPreflightCompactsAbortedHighUsageResponse(t *testing.T) {
	...
	created := engine.NewAgent(nil, engine.WithInitialState(engine.AgentState{Model: &model, Messages: messages}))
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{
		Agent: created, SessionManager: manager, Settings: settings,
		GetAPIKey: func(context.Context, ai.ProviderID) (*string, error) {
			key := "key"
