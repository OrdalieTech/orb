...
func (tool seamStubTool) Spec() engine.AgentToolSpec {
	return engine.AgentToolSpec{Name: tool.name, Description: "stub", Parameters: jsonschema.Schema(`{"type":"object"}`)}
}

func (seamStubTool) Execute(context.Context, string, any, engine.AgentToolUpdateCallback) (engine.AgentToolResult, error) {
	return engine.AgentToolResult{Content: ai.ToolResultContent{&ai.TextContent{Text: "ok"}}}, nil
}

func (seamStubTool) RenderCall(any) string { return "stub call" }
func (seamStubTool) RenderResult(engine.AgentToolResult) string {
	return "stub result"
}
...
func TestGetToolDefinitionKeepsCustomRenderersReachable(t *testing.T) {
	...
	if err := registry.Register("<inline:renderers>", func(api extensions.API) error {
		api.RegisterTool(extensions.ToolDefinition{
			...
			RenderResult: func(engine.AgentToolResult, extensions.ToolRenderResultOptions, extensions.Theme, extensions.ToolRenderContext) extensions.Component {
	...
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{
		...
		ExtensionRegistry: registry, BaseTools: []engine.AgentTool{base},
	...
	result := definition.RenderResult(engine.AgentToolResult{}, extensions.ToolRenderResultOptions{}, nil, extensions.ToolRenderContext{})
...
func TestRegisteredToolWithoutExtensionsScansAgentTools(t *testing.T) {
	...
	created := engine.NewAgent(nil, engine.WithInitialState(engine.AgentState{
		Messages: engine.AgentMessages{}, Tools: []engine.AgentTool{base},
	}))
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{Agent: created, SessionManager: manager, Settings: settings})
	if err != nil {
		t.Fatal(err)
...
func TestPendingMessagesSnapshotsQueueOrderAndCopies(t *testing.T) {
	...
	if err := runtime.handleAgentEvent(context.Background(), engine.MessageStartEvent{Message: userMessage("first steer")}); err != nil {
		t.Fatal(err)
	}
	snapshot = runtime.PendingMessages()
