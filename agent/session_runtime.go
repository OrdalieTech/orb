...
func NewSessionRuntime(runtimeConfig SessionRuntimeConfig) (*SessionRuntime, error) {
	if runtimeConfig.Agent == nil || runtimeConfig.SessionManager == nil || runtimeConfig.Settings == nil {
		return nil, errors.New("agent: session runtime requires agent, session manager, and settings")
	}
...
func (runtime *SessionRuntime) handleAgentEvent(ctx context.Context, event engine.AgentEvent) error {
	...
	ephemeral := engine.IsEphemeralAgentEvent(ctx)
	event = runtime.extensionLifecycleEvent(ctx, event)
	if ended, ok := event.(engine.AgentEndEvent); ok {
...
func (runtime *SessionRuntime) willRetry(messages engine.AgentMessages) bool {
	settings := runtime.settings.GetRetrySettings()
	runtime.mu.Lock()
	attempt := runtime.retryAttempt
	runtime.mu.Unlock()
...
func decodeSessionMessage(raw json.RawMessage) engine.AgentMessage {
	if len(raw) == 0 {
		return nil
	}
...
func asAssistant(message engine.AgentMessage) *ai.AssistantMessage {
...
func userMessageText(message engine.AgentMessage) string {
...
func estimateAllTokens(messages engine.AgentMessages) int64 {
	var total int64
