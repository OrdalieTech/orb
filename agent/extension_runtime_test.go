...
func TestSessionRuntimeWiresExtensionHooksAndLifecycle(t *testing.T) {
	...
	if err := registry.Register("<inline:wire>", func(api extensions.API) error {
		...
		api.RegisterTool(extensions.ToolDefinition{
			...
			Execute: func(context.Context, string, any, engine.AgentToolUpdateCallback, extensions.Context) (engine.AgentToolResult, error) {
				...
				return engine.AgentToolResult{Content: ai.ToolResultContent{&ai.TextContent{Text: "original"}}}, nil
			},
		})
		return nil
	}); err != nil {
	...
	baseRan := false
	baseBash := engine.AgentToolFunc{
		AgentToolSpec: engine.AgentToolSpec{Name: "bash", Label: "bash", Description: "base", Parameters: jsonschema.Schema(`{"type":"object","properties":{}}`)},
	...
	created := engine.NewAgent(
		stream, engine.WithInitialState(engine.AgentState{SystemPrompt: "base", Model: provider.GetModel(), Tools: []engine.AgentTool{baseBash}}),
	...
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{
		...
		ExtensionMode: extensions.ModePrint, BaseTools: []engine.AgentTool{baseBash},
	...
	runtime.Subscribe(func(event any) {
		if ended, ok := event.(engine.MessageEndEvent); ok {
...
func TestExtensionCommandInputAndNativeResourcesShareUpstreamOrder(t *testing.T) {
	...
	readTool := engine.AgentToolFunc{AgentToolSpec: engine.AgentToolSpec{
	...
	created := engine.NewAgent(
		provider.StreamSimple, engine.WithInitialState(engine.AgentState{
			SystemPrompt: BuildSystemPrompt(promptOptions), Model: provider.GetModel(), Tools: []engine.AgentTool{readTool},
		}),
	)
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{
		Agent: created, SessionManager: manager, Settings: settings, ExtensionRegistry: registry,
		SlashResolver: &SlashResolver{Skills: []Skill{baseSkill}, PromptTemplates: []PromptTemplate{basePrompt}},
		BaseTools:     []engine.AgentTool{readTool}, InitialActiveToolNames: []string{"read"}, SystemPromptOptions: &promptOptions,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDynamicActiveToolsReportOnlyAdditions(t *testing.T) {
	...
	created := engine.NewAgent(nil, engine.WithInitialState(engine.AgentState{SystemPrompt: "base"}))
...
func TestExtensionToolOverrideReplacesBuiltInPromptMetadata(t *testing.T) {
	...
	base := engine.AgentToolFunc{AgentToolSpec: engine.AgentToolSpec{Name: "read", Description: "base", Parameters: jsonschema.Schema(`{}`)}}
	...
	created := engine.NewAgent(nil, engine.WithInitialState(engine.AgentState{SystemPrompt: BuildSystemPrompt(promptOptions), Tools: []engine.AgentTool{base}}))
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{
		Agent: created, SessionManager: manager, Settings: settings, ExtensionRegistry: registry,
		BaseTools: []engine.AgentTool{base}, InitialActiveToolNames: []string{"read"}, SystemPromptOptions: &promptOptions,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestExtensionToolsApplyAllowlistAndDenylist(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			base := engine.AgentToolFunc{AgentToolSpec: engine.AgentToolSpec{Name: "read", Description: "base", Parameters: jsonschema.Schema(`{}`)}}
			created := engine.NewAgent(nil, engine.WithInitialState(engine.AgentState{SystemPrompt: "base", Tools: []engine.AgentTool{base}}))
			runtime, err := NewSessionRuntime(SessionRuntimeConfig{
				Agent: created, SessionManager: manager, Settings: settings, ExtensionRegistry: registry,
				BaseTools: []engine.AgentTool{base}, InitialActiveToolNames: []string{"read"},
...
func TestConcurrentUserBashCancellationTracksEveryExecution(t *testing.T) {
	...
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{
		Agent: engine.NewAgent(nil), SessionManager: manager, Settings: settings, ExtensionRegistry: registry,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestExtensionUserMessageRunsInputHookBeforeStreamingQueue(t *testing.T) {
	...
	created := engine.NewAgent(provider.StreamSimple, engine.WithInitialState(engine.AgentState{SystemPrompt: "base", Model: provider.GetModel()}), engine.WithConvertToLLM(ConvertToLLM))
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{Agent: created, SessionManager: manager, Settings: settings, ExtensionRegistry: registry})
	if err != nil {
		t.Fatal(err)
	}
...
func TestNormalizeExtensionMessageFillsMissingCoreContent(t *testing.T) {
	tests := []struct {
		...
		valid   func(engine.AgentMessage) bool
	}{
		{
			name: "user", message: &ai.UserMessage{},
			valid: func(message engine.AgentMessage) bool {
...
func TestExtensionCompactionHooksCanProvideSummary(t *testing.T) {
	...
	provider := faux.New()
	created := engine.NewAgent(nil, engine.WithInitialState(engine.AgentState{Model: provider.GetModel()}))
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{
		Agent: created, SessionManager: manager, Settings: settings, ExtensionRegistry: registry,
	})
	if err != nil {
...
func TestExtensionThinkingLevelClampsAndSkipsDuplicatePersistence(t *testing.T) {
	...
	created := engine.NewAgent(nil, engine.WithInitialState(engine.AgentState{
		Model: &ai.Model{Reasoning: false}, ThinkingLevel: engine.ThinkingOff,
	}))
	...
	defer runtime.Dispose()
	if err := runtime.setExtensionThinkingLevel(engine.ThinkingHigh); err != nil {
		t.Fatal(err)
	}
	if got := runtime.State().ThinkingLevel; got != engine.ThinkingOff || len(manager.GetEntries()) != 0 {
		t.Fatalf("non-reasoning state = %q, entries = %d", got, len(manager.GetEntries()))
	}
	runtime.agent.SetModel(&ai.Model{Reasoning: true})
	if err := runtime.setExtensionThinkingLevel(engine.ThinkingXHigh); err != nil {
		t.Fatal(err)
	}
	if got := runtime.State().ThinkingLevel; got != engine.ThinkingHigh || len(manager.GetEntries()) != 1 {
...
func TestModelAndThinkingMutationsAreTransientAndEmitExtensionEvents(t *testing.T) {
	...
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{
		Agent:          engine.NewAgent(nil, engine.WithInitialState(engine.AgentState{Model: &modelA, ThinkingLevel: ai.ModelThinkingLow})),
...
func TestNoExtensionsLeavesRuntimeSeamUnallocated(t *testing.T) {
	cwd := t.TempDir()
	manager, settings := extensionRuntimeDependencies(t, cwd)
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{Agent: engine.NewAgent(nil), SessionManager: manager, Settings: settings})
	if err != nil {
		t.Fatal(err)
	}
	defer runtime.Dispose()
...
func TestExtensionToolCallInputMutatesExecutionNotTheRecord(t *testing.T) {
	...
	var executed any
	tool := engine.AgentToolFunc{
		AgentToolSpec: engine.AgentToolSpec{
			Name:       "echo",
			Parameters: jsonschema.Schema(`{"type":"object","properties":{"path":{"type":"string"}}}`),
		},
		Run: func(_ context.Context, _ string, args any, _ engine.AgentToolUpdateCallback) (engine.AgentToolResult, error) {
			executed = args
			return engine.AgentToolResult{Content: ai.ToolResultContent{&ai.TextContent{Text: "done"}}}, nil
		},
	}
	provider := faux.New()
	provider.SetResponses([]faux.ResponseStep{
	...
	created := engine.NewAgent(
		...
		engine.WithInitialState(engine.AgentState{Model: provider.GetModel(), Tools: []engine.AgentTool{tool}}),
		engine.WithConvertToLLM(ConvertToLLM),
	)
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{
		Agent: created, SessionManager: manager, Settings: settings, StreamFn: provider.StreamSimple,
		ExtensionRegistry: registry, BaseTools: []engine.AgentTool{tool}, InitialActiveToolNames: []string{"echo"},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestExtensionQueuedSlashCommandFollowUpStaysRawText(t *testing.T) {
	...
	waitTool := engine.AgentToolFunc{
		...
		Run: func(ctx context.Context, _ string, _ any, _ engine.AgentToolUpdateCallback) (engine.AgentToolResult, error) {
			...
			return engine.AgentToolResult{Content: ai.ToolResultContent{&ai.TextContent{Text: "released"}}}, nil
		},
	}
	provider := faux.New()
	provider.SetResponses([]faux.ResponseStep{
	...
	created := engine.NewAgent(
		provider.StreamSimple, engine.WithInitialState(engine.AgentState{SystemPrompt: "test", Model: provider.GetModel(), Tools: []engine.AgentTool{waitTool}}),
		engine.WithConvertToLLM(ConvertToLLM),
	)
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{
		Agent: created, SessionManager: manager, Settings: settings,
		ExtensionRegistry: registry, ExtensionMode: extensions.ModePrint,
		BaseTools: []engine.AgentTool{waitTool}, InitialActiveToolNames: []string{"wait"},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	runtime.Subscribe(func(event any) {
		if started, ok := event.(engine.ToolExecutionStartEvent); ok && started.ToolName == "wait" {
			select {
...
func TestSessionStartEvent(t *testing.T) {
	for _, test := range []struct {
		name  string
		start *extensions.SessionStartEvent
		want  extensions.SessionStartReason
	}{
		{name: "NonDefault", start: &extensions.SessionStartEvent{Reason: extensions.SessionStartResume}, want: extensions.SessionStartResume},
		{name: "DefaultIsStartup", want: extensions.SessionStartStartup},
	} {
		t.Run(test.name, func(t *testing.T) {
			cwd := t.TempDir()
			manager, settings := extensionRuntimeDependencies(t, cwd)
			...
			runtime, err := NewSessionRuntime(SessionRuntimeConfig{
				...
				ExtensionRegistry: registry, SessionStartEvent: test.start,
			})
			if err != nil {
				t.Fatal(err)
			}
			defer runtime.Dispose()
			if receivedReason != test.want {
				t.Fatalf("session_start reason = %q, want %q", receivedReason, test.want)
			}
		})
	}
}
...
func TestSessionRuntimeReloadClearsHooksRemovedByFreshRegistry(t *testing.T) {
	...
	tool := engine.AgentToolFunc{
		AgentToolSpec: engine.AgentToolSpec{Name: "noop", Parameters: jsonschema.Schema(`{"type":"object"}`)},
		Run: func(context.Context, string, any, engine.AgentToolUpdateCallback) (engine.AgentToolResult, error) {
			return engine.AgentToolResult{Content: ai.ToolResultContent{&ai.TextContent{Text: "done"}}}, nil
		},
	}
	...
	created := engine.NewAgent(
		stream, engine.WithInitialState(engine.AgentState{Model: provider.GetModel(), Tools: []engine.AgentTool{tool}}),
		engine.WithConvertToLLM(ConvertToLLM),
	)
	runtime, err := NewSessionRuntime(SessionRuntimeConfig{
		Agent: created, SessionManager: manager, Settings: settings, StreamFn: stream,
		ExtensionRegistry: registry, BaseTools: []engine.AgentTool{tool}, InitialActiveToolNames: []string{"noop"},
	})
	if err != nil {
		t.Fatal(err)
	}
