...
func main() {
	...
	result2, err := agent.NewAgentSession(agent.AgentSessionOptions{
		StreamFn: provider.StreamSimple,
		Model:    provider.GetModel(),
	...
	result3, err := agent.NewAgentSession(agent.AgentSessionOptions{
		StreamFn:     provider.StreamSimple,
		Model:        provider.GetModel(),
	...
	// No tools at all.
	result4, err := agent.NewAgentSession(agent.AgentSessionOptions{
		StreamFn: provider.StreamSimple,
		Model:    provider.GetModel(),
		NoTools:  "all",
	})
	...
	result5, err := agent.NewAgentSession(agent.AgentSessionOptions{
		CWD:      "/tmp",
		StreamFn: provider.StreamSimple,
		Model:    provider.GetModel(),
...
func names(tools []engine.AgentTool) []string {
