...
func main() {
	...
	root, err := os.MkdirTemp("", "orb-sdk-runtime-")
	if err != nil {
		log.Fatal(err)
	}
	defer func() {
	...
	createRuntime := agent.CreateAgentSessionRuntimeFactory(func(_ context.Context, options agent.AgentSessionOptions) (*agent.AgentSessionResult, error) {
	...
	runtime, err := agent.NewAgentSessionRuntime(ctx, agent.AgentSessionOptions{
	...
	bindSession := func(session *agent.AgentSession) error {
		if unsubscribe != nil {
			unsubscribe()
		}
		...
		unsubscribe = session.Subscribe(func(event any) {
			if queue, ok := event.(agent.QueueUpdateEvent); ok {
