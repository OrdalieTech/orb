...
func main() {
	...
	loader1, err := agent.NewDefaultResourceLoader(agent.DefaultResourceLoaderOptions{
		CWD:      cwd,
		AgentDir: agentDir,
	...
	loader2, err := agent.NewDefaultResourceLoader(agent.DefaultResourceLoaderOptions{
		CWD:      cwd,
		AgentDir: agentDir,
...
func newSession(cwd string, provider *faux.Provider, loader agent.ResourceLoader) *agent.AgentSession {
...
func printAssistantText(messages engine.AgentMessages) {
