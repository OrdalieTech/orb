...
func TestToolUsesExplicitAttachmentAndValidatesCalls(t *testing.T) {
	...
	calls := 0
	tool, err := NewTool(func(_ context.Context, peer string, call bridge.Call) (json.RawMessage, error) {
		calls++
	...
	result, err := tool.Execute(t.Context(), "call", map[string]any{"peer_id": "remote", "call": bridge.Call{InstanceID: protocol.NewID(), Service: protocol.Service, Method: "inspect", Args: bridge.JSON(struct{}{})}}, nil)
