...
func ConvertToLLM(_ context.Context, messages engine.AgentMessages) (ai.MessageList, error) {
	...
	for _, message := range messages {
		...
		case "user", "assistant", "toolResult":
			...
			if err != nil {
				return nil, fmt.Errorf("agent: decode standard message: %w", err)
			}
			converted = append(converted, standard)
		case "custom":
			user, err := customUserMessage(custom.Content, custom.Timestamp)
			if err != nil {
				return nil, fmt.Errorf("agent: decode custom message: %w", err)
			}
...
func ConvertToLLMWithBlockImages(blockImages func() bool) engine.ConvertToLLMFunc {
	return func(ctx context.Context, messages engine.AgentMessages) (ai.MessageList, error) {
