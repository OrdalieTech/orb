...
func Extension(client *http.Client) extensions.Factory {
	if client == nil {
	...
	return func(api extensions.API) error {
		api.RegisterTool(extensions.ToolDefinition{
			...
			Execute: func(ctx context.Context, _ string, raw any, _ engine.AgentToolUpdateCallback, _ extensions.Context) (engine.AgentToolResult, error) {
				...
				if input.Query == "" {
					return engine.AgentToolResult{}, fmt.Errorf("web_search: query is required")
				}
				results, err := searchWeb(ctx, client, input.Query)
				if err != nil {
					return engine.AgentToolResult{}, err
				}
				return toolutil.TextResult(truncateWeb(formatSearchResults(results))), nil
			},
		})
		api.RegisterTool(extensions.ToolDefinition{
			...
			Execute: func(ctx context.Context, _ string, raw any, _ engine.AgentToolUpdateCallback, _ extensions.Context) (engine.AgentToolResult, error) {
				...
				return toolutil.TextResult(truncateWeb(text)), nil
			},
		})
		return nil
	}
