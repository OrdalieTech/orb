...
func TestFileStoreAppendGetQueryAndDelete(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "tag AND", filter: memory.Filter{Tags: []string{"go", "sdk"}}, want: []string{"ALPHA third", "Alpha first"}},
		{name: "time window inclusive", filter: memory.Filter{Since: base.Add(time.Minute), Until: base.Add(2 * time.Minute)}, want: []string{"ALPHA third", "beta second"}},
		{name: "case insensitive substring", filter: memory.Filter{Contains: "aLpHa"}, want: []string{"ALPHA third", "Alpha first"}},
		{name: "limit", filter: memory.Filter{Limit: 1}, want: []string{"ALPHA third"}},
	}
	...
	for index := 0; index < 101; index++ {
		if _, err := store.Append(ctx, memory.Item{Time: base.Add(time.Duration(index+3) * time.Minute), Content: "bulk"}); err != nil {
			t.Fatal(err)
		}
	}
	items, err := store.Query(ctx, memory.Filter{Limit: 1000})
	...
	items, err = store.Query(ctx, memory.Filter{Contains: "third"})
...
func TestFileStoreConcurrentAccess(t *testing.T) {
	...
	for worker := 0; worker < workers; worker++ {
		...
		go func() {
			...
			for index := 0; index < perWorker; index++ {
				if _, appendErr := store.Append(context.Background(), memory.Item{Content: "concurrent"}); appendErr != nil {
					t.Errorf("Append: %v", appendErr)
					return
				}
				if _, queryErr := store.Query(context.Background(), memory.Filter{}); queryErr != nil {
	...
	group.Wait()
	items, err := store.Query(context.Background(), memory.Filter{})
...
func TestFileStoreSkipsCorruptLines(t *testing.T) {
	...
	if _, err := store.Append(context.Background(), memory.Item{Content: "before"}); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := store.Append(context.Background(), memory.Item{Content: "after"}); err != nil {
		t.Fatal(err)
	}
	items, err := store.Query(context.Background(), memory.Filter{})
...
func TestFileStoreTransactionKeepsCompoundWritesAtomic(t *testing.T) {
	...
	for range 2 {
		...
		go func() {
			defer group.Done()
			if txErr := store.Transact(context.Background(), func(tx memory.Store) error {
				items, queryErr := tx.Query(context.Background(), memory.Filter{Contains: "same fact"})
				if queryErr != nil || len(items) > 0 {
					return queryErr
				}
				_, appendErr := tx.Append(context.Background(), memory.Item{Content: "same fact"})
				return appendErr
			}); txErr != nil {
	...
	if err := store.Transact(context.Background(), func(tx memory.Store) error {
		if _, appendErr := tx.Append(context.Background(), memory.Item{Content: "replacement"}); appendErr != nil {
			return appendErr
		}
	...
	if err := store.Transact(context.Background(), func(tx memory.Store) error {
		if _, appendErr := tx.Append(context.Background(), memory.Item{Content: "rolled back"}); appendErr != nil {
			return appendErr
		}
		return stop
	}); !errors.Is(err, stop) {
	...
	items, err = store.Query(context.Background(), memory.Filter{Contains: "rolled back"})
	if err != nil || len(items) != 0 {
