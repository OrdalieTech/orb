...
func TestManagerRegistersExecutesAndStreamsExampleTool(t *testing.T) {
	...
	updates := make(chan engine.AgentToolResult, 1)
	result, err := extensions.WrapRegisteredTool(tools[0], runner).Execute(
		context.Background(), "call-1", map[string]any{"text": "hello"}, func(update engine.AgentToolResult) { updates <- update },
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestManagerDeliversWirePriorProgressBeforeAgentSettlesTool(t *testing.T) {
	...
	go func() {
		_, err := engine.RunLoop(
			context.Background(),
			engine.AgentMessages{&ai.UserMessage{Content: ai.NewUserText("work")}},
			engine.AgentContext{Tools: []engine.AgentTool{tool}},
			engine.AgentLoopConfig{Model: &ai.Model{ID: "test-model", API: "test", Provider: "test"}},
			func(_ context.Context, event engine.AgentEvent) error {
				eventsMu.Lock()
...
func TestManagerDoesNotCrossWireOverlappingReusedToolCallIDs(t *testing.T) {
	...
	defer releaseSecond()
	go func() {
		result, err := tool.Execute(context.Background(), "reused", map[string]any{"label": "first"}, func(update engine.AgentToolResult) {
	...
	go func() {
		result, err := tool.Execute(context.Background(), "reused", map[string]any{"label": "second"}, func(update engine.AgentToolResult) {
...
func TestManagerDoesNotCrossWireLateProgressAfterSequentialIDReuse(t *testing.T) {
	...
	first, err := tool.Execute(context.Background(), "reused", map[string]any{"label": "first"}, func(update engine.AgentToolResult) {
		firstUpdates <- contentText(update.Content)
	})
...
func TestManagerDrainsStreamableHTTPProgressBeforeExecuteReturns(t *testing.T) {
	...
	go func() {
		result, err := tool.Execute(context.Background(), "http-progress", map[string]any{}, func(update engine.AgentToolResult) {
...
func TestManagerDrainsJSONResponseStandaloneProgressBeforeExecuteReturns(t *testing.T) {
	...
	go func() {
		result, err := tool.Execute(context.Background(), "http-json-progress", map[string]any{}, func(update engine.AgentToolResult) {
			updates <- contentText(update.Content)
			close(progressStarted)
			<-releaseProgress
		})
...
func TestManagerNoProgressDoesNotEnterAnArrivalWait(t *testing.T) {
	...
	defer closeManager(t, manager)
	registration := &progressRegistration{token: "none", update: func(engine.AgentToolResult) {}}
...
func TestManagerSealsRegistrationWhileObservedProgressDrains(t *testing.T) {
	...
	registration := &progressRegistration{
		token: "sealed",
		update: func(update engine.AgentToolResult) {
			updates <- contentText(update.Content)
		},
	}
	manager.progress[registration.token] = registration
...
func TestManagerIgnoresStandaloneProgressObservedAfterCallSettles(t *testing.T) {
	...
	result, err := extensions.WrapRegisteredTool(registered, runner).Execute(
		context.Background(), "late-progress", map[string]any{}, func(update engine.AgentToolResult) {
			updates <- contentText(update.Content)
		},
	)
...
func TestManagerMatchesSDKWhitespaceParsingBeforeExecuteReturns(t *testing.T) {
	...
	go func() {
		result, err := tool.Execute(context.Background(), "http-whitespace-progress", map[string]any{}, func(engine.AgentToolResult) {
			close(progressStarted)
			<-releaseProgress
		})
		done <- execution{result: result, err: err}
...
func TestManagerMalformedSSEDoesNotLeavePendingProgress(t *testing.T) {
	...
	go func() {
		result, err := tool.Execute(ctx, "http-malformed-progress", map[string]any{}, func(engine.AgentToolResult) {})
		done <- execution{result: result, err: err}
	}()
	select {
