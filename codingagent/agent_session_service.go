...
func (service *ExtensionAgentSessionService) callbackToolDefinition(
	...
	return extensions.ToolDefinition{
		...
		Execute: func(
			...
			if execute == nil {
				return agent.AgentToolResult{}, fmt.Errorf("codingagent: session tool %s has no host transport", name)
			}
			...
			if err := json.Unmarshal(raw, &result); err != nil {
				return agent.AgentToolResult{}, fmt.Errorf("codingagent: decode session tool %s result: %w", name, err)
			}
			return result, nil
		},
	}
...
func orderedSessionToolParams(session *AgentSession, toolCallID string, params any) any {
	...
	if state.StreamingMessage != nil {
		messages = append(append(agent.AgentMessages(nil), messages...), state.StreamingMessage)
	}
