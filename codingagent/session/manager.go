...
// BranchHasEntryType reports whether any entry on the current branch has the
// given type, without cloning the branch.
func (manager *SessionManager) BranchHasEntryType(entryType string) bool {
	if manager.harnessStorage != nil {
...
// FindBranchEntry returns a deep clone of the last entry on the current
// branch appended strictly after the entry afterID (the whole branch when
// afterID is absent) for which match returns true, or nil when none exists.
// match must not retain message: the bytes alias manager state during the call.
func (manager *SessionManager) FindBranchEntry(afterID string, match func(entryType string, message json.RawMessage) bool) *SessionEntry {
	if manager.harnessStorage != nil {
		...
		for index := len(branch) - 1; index >= start; index-- {
			if match(branch[index].Type, branch[index].Message) {
				entry := branch[index]
				return &entry
			}
		}
	...
	for current := manager.byID[*manager.leafID]; current != nil; {
		...
		if match(current.Type, current.Message) {
