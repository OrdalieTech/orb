...
func BuildSystemPrompt(options SystemPromptOptions) string {
	...
	prompt := fmt.Sprintf(`You are an expert problem-solving assistant operating inside Orb, a general-purpose agent harness for work and software development. You help users investigate, plan, create, and complete tasks using the available tools, including working with files, executing commands, and editing code or documents.
...
- Always read Orb documentation files completely and follow links to related docs (e.g., tui.md for TUI API details)`, toolsList, strings.Join(formattedGuidelines, "\n"), readmePath, docsPath, examplesPath)
