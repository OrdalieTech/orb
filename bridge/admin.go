...
func (b *Bridge) Admin(_ context.Context, method string, params json.RawMessage) (json.RawMessage, error) {
	...
	case "approve":
		...
		return JSON(struct{}{}), b.Approve(p.ID, p.Claimant)
	case "block":
		var p struct {
		...
		return JSON(struct{}{}), b.Block(p.Peer)
	case "grant":
		var p Grant
		if err := protocol.Decode(params, &p); err != nil {
			return nil, err
		}
		return JSON(struct{}{}), b.AddGrant(p)
	case "revoke":
		var p struct {
	...
	case "scope":
		...
		return JSON(struct{}{}), b.SetScope(p.ID, p.Peers)
	case "group":
		var p struct {
...
func (b *Bridge) Outbound(instance, peer string, call Call) (Subject, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	p := Principal{PeerID: b.PeerID(), Subject: Subject{Kind: "instance", InstanceID: instance}}
	if !b.allowed(p, call.InstanceID, permission(call.Method), peer) {
		return Subject{}, Fail("unauthorized")
	}
	if _, ok := b.active[instance]; !ok {
		return Subject{}, Fail("unavailable")
	}
	return p.Subject, nil
}
func (b *Bridge) Authorize(r Request) bool {
	b.mu.Lock()
	defer b.mu.Unlock()
