...
func TestScopedThreeBridgeReconciliation(t *testing.T) {
	...
	if _, err = outsider.MergeContact(b.PeerID(), record); connect.Code(err) != "unauthorized" {
		t.Fatal(err)
	}
...
func TestWithdrawalFloorSurvivesRestartAndRejectsEquivocation(t *testing.T) {
	...
	if _, err = a.MergeContact(c.PeerID(), conflict); connect.Code(err) != "identity_conflict" {
		t.Fatal(err)
	}
	withdrawn, err := c.PublishContact(scope, "C", nil, true)
	if err != nil {
