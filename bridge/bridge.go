...
func Open(store connect.Store, create bool) (*Bridge, error) {
	if store == nil {
...
func (b *Bridge) Principal() connect.Principal {
	return connect.Principal{PeerID: b.PeerID(), Subject: connect.Subject{Kind: "controller"}}
}
...
func (b *Bridge) AddGrant(g Grant) error {
	...
	for _, old := range b.state.Grants {
		if old.ID == g.ID {
			return connect.Fail("identity_conflict")
		}
	}
	b.state.Grants = append(b.state.Grants, g)
	return b.save()
}
func (b *Bridge) allowed(p connect.Principal, id, permission, destination string) bool {
...
func (b *Bridge) Allowed(p connect.Principal, id, permission string) bool {
	b.mu.Lock()
	defer b.mu.Unlock()
	return b.allowed(p, id, permission, "")
}
func (b *Bridge) Catalog(p connect.Principal) []Instance {
	b.mu.Lock()
	defer b.mu.Unlock()
	out := []Instance{}
	for id, r := range b.state.Instances {
...
func (b *Bridge) Enroll(alias, group string) (Instance, string, error) {
	...
	if len(alias) > 128 || len(b.state.Instances) >= 4096 {
		return Instance{}, "", connect.Fail("resource_exhausted")
	}
	if b.state.Groups[group] == "" {
		return Instance{}, "", connect.Fail("not_found")
	}
	if alias != "" {
		for _, r := range b.state.Instances {
			if r.Alias == alias {
				return Instance{}, "", connect.Fail("identity_conflict")
			}
		}
	}
	token := secret()
...
func (b *Bridge) Attach(id, credential, boot string, endpoint connect.Endpoint) (string, error) {
...
func (b *Bridge) Invite(grants []Grant) (Invitation, error) {
	...
	if len(b.state.Invitations) >= 128 {
		return Invitation{}, connect.Fail("resource_exhausted")
	}
...
func (b *Bridge) Claim(peer, id, token string, locators ...string) (Invitation, error) {
	...
	if !ok || b.state.Blocked[peer] || i.Expires < time.Now().Unix() || !matches(token, i.Hash) {
		return Invitation{}, connect.Fail("unauthorized")
	}
	if i.Claimant != "" && i.Claimant != peer {
		return Invitation{}, connect.Fail("identity_conflict")
	}
	if len(locators) > 0 {
...
func (b *Bridge) PairStatus(peer, id string) (Invitation, error) {
	...
	if !ok || i.Claimant != peer || b.state.Blocked[peer] || (i.Status != "approved" && i.Expires < time.Now().Unix()) {
		return Invitation{}, connect.Fail("not_found")
	}
	return cloneInvitation(i.Invitation), nil
}
func (b *Bridge) Approve(id, claimant string) error {
	...
	for _, g := range i.Grants {
		g.Principal = connect.Principal{PeerID: claimant, Subject: connect.Subject{Kind: "controller"}}
		g, err := b.prepareGrant(g)
		if err != nil {
			return err
		}
...
func (b *Bridge) Call(ctx context.Context, p connect.Principal, call connect.Call) (json.RawMessage, error) {
	if err := connect.ValidateCall(call); err != nil {
		return nil, err
	}
	b.mu.Lock()
	...
	return r.endpoint.Invoke(ctx, "call", connect.JSON(connect.Request{PageLimit: protocol.PageLimit(ctx), Principal: p, Generation: r.generation, Call: call}))
}
